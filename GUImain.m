function GUImain(refImg,filterType,quality,colorSpace,PreFilterQuality,PreFilterAmount,subImSize,neighbourhood,resultFILENAME)
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
%   neighbourhood = Radius of the square neighborhood for nonreocurring images
...
s_h = subImSize;                                           % Dimensions of a single small image
s_w = subImSize;                                           %
...
for index = 1:bs_h
    ...
    neighborhood_index              = neighborhoodFilter(data_index,index,index_img,neighbourhood);
    rough_index                     = RoughFilter(target,data,neighborhood_index,PreFilterAmount,PreFilterQuality); %fast (mean) or best (pixel difference)
    switch filterType
        case 'MSE'
            Filter_index            = MSEFilter(target,data,rough_index,1,quality);  %fast (pixel difference) or slow (MSE)
        case 'Hist'
            Filter_index            = HistFilter(target,data,rough_index,1,quality);
        case 'OCCD'
            Filter_index            = OCCDFilter(target,data,rough_index,1,quality,colorSpace,s_h,s_w);
        case 'Edge'
            Filter_index            = EdgeFilter(target,data,rough_index,1,quality,'intersection',colorSpace);
        otherwise
...
imshow(new_big_show);
imwrite(new_big_show,resultFILENAME)
